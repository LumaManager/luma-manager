...
import { DistributionDrawer } from "@/components/agenda/distribution-drawer";
...
export function Topbar({ bootstrap }: TopbarProps) {
  ...
  const isOnAgenda = pathname.startsWith("/app/agenda");
    ...
    <>
      <header className="sticky top-0 z-20 flex items-center justify-between gap-4 border-b border-[var(--color-border)] bg-[rgba(246,241,232,0.92)] px-8 py-5 backdrop-blur">
        <div className="space-y-1.5">
          ...
          {activationMode && (
            <div className="flex items-center gap-1.5 text-xs text-[var(--color-text-muted)]" title="Conclua a ativação para liberar agenda, pacientes e cobrança.">
              <AlertTriangle className="h-3.5 w-3.5 text-[var(--color-accent)]" />
              <span>Conta em ativação</span>
            </div>
          )}
        </div>

        <div className="flex flex-wrap items-center justify-end gap-2">
          {isOnAgenda && !activationMode && (
            <button
              ...
              onClick={() => setIsDistributionDrawerOpen(true)}
              type="button"
            >
              Distribuir links
            </button>
      ...
      <DistributionDrawer
        isOpen={isDistributionDrawerOpen}
        onClose={() => setIsDistributionDrawerOpen(false)}
      />
    </>
  );
}
