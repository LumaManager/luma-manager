...
import { ClinicalReviewQueuePageView } from "@/components/clinical-review/clinical-review-queue-page";
...
export default async function ClinicalReviewPage({
  ...
  const data = await apiFetch<ClinicalReviewQueueResponse>(
    `/v1/clinical-review${query ? `?${query}` : ""}`,
    {
      token
    }
  );

  return <ClinicalReviewQueuePageView initialData={data} />;
}
